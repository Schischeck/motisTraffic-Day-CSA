  ...
  edge_cost get_edge_cost(time start_time,
    ...
    switch (m_.type_) {
      ...
      case PERIODIC_MUMO_EDGE:
        return edge_cost(
            calc_time_off(m_.foot_edge_.interval_begin_,
                          m_.foot_edge_.interval_end_, start_time) +
      ...
      if (m_.route_edge_.conns_.empty()) {
        ...
        return edge_cost(
            ...
            false, std::begin(m_.route_edge_.conns_)->full_con_->price_);
               ...
               m_.type_ == AFTER_TRAIN_FWD_EDGE ||
               m_.type_ == AFTER_TRAIN_BWD_EDGE || m_.type_ == BWD_EDGE) {
    ...
    if (Dir == search_dir::FWD) {
      ...
      if (m_.route_edge_.traffic_days_ != nullptr) {
        ...
        for (int day_idx = start_time.day(); day_idx <= last_day; ++day_idx) {
          has_traffic |= m_.route_edge_.traffic_days_->test(
              day_idx);  // warum nicht abbrdechen wenn has_traffic = true
        }
        if (!has_traffic) {
          return {nullptr, 0};
        }
...
private:
  ...
  edge_cost calc_cost_hotel_edge(time const start_time) const {
    uint16_t const offset =
        start_time.mam() < m_.hotel_edge_.checkout_time_ ? 0 : MINUTES_A_DAY;
    auto const duration = std::max(
        m_.hotel_edge_.min_stay_duration_,
        static_cast<uint16_t>((m_.hotel_edge_.checkout_time_ + offset) -
                              (start_time.mam())));
    return edge_cost(time(duration), false, m_.hotel_edge_.price_);
  }
};
...
inline edge make_time_dependent_mumo_edge(node* from, node* to, time time_cost,
                                          uint16_t price, int mumo_id,
                                          time interval_begin,
                                          time interval_end) {
...
inline edge make_periodic_mumo_edge(node* from, node* to, time time_cost,
                                    uint16_t price, int mumo_id,
