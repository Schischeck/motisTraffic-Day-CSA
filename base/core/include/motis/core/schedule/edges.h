...
private:
  ...
  edge_cost edge_cost_for_time_dependent_mumo(time const start_time) const {
    auto const t =
        m_.foot_edge_.periodical_validity_ ? start_time % 1440 : start_time;
    if (t >= m_.foot_edge_.valid_from_ && t <= m_.foot_edge_.valid_to_) {
      return edge_cost(m_.foot_edge_.time_cost_, m_.foot_edge_.transfer_,
                       m_.foot_edge_.price_, m_.foot_edge_.slot_);
    } else {
      return NO_EDGE;
...
inline edge make_time_dependent_mumo_edge(
  ...
  edge e(from, to, edge::TIME_DEPENDENT_MUMO_EDGE, time_cost, price, false,
         slot);
  e.m_.foot_edge_.valid_from_ = valid_from;
  e.m_.foot_edge_.valid_to_ = valid_to;
  e.m_.foot_edge_.periodical_validity_ = periodical_validity;
  return e;
}
