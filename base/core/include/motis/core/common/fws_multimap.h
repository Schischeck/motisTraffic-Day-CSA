...
struct fws_multimap_entry {
  ...
  fws_multimap_entry(std::vector<T> const& data,
                     std::vector<Index> const& index, Index key)
      : data_(data), index_start(index[key]), index_end(index[key + 1]) {}

  fws_multimap_entry(std::vector<T> const& data, Index start_index,
                     Index end_index)
...
struct shared_idx_fws_multimap {
  ...
  explicit shared_idx_fws_multimap(std::vector<Index> const& base_index)
  ...
  std::vector<Index> const& base_index_;
  std::vector<T> data_;
};

template <typename T, typename Index = uint64_t>
struct nested_fws_multimap {
  explicit nested_fws_multimap(std::vector<Index> const& base_index)
      : base_index_(base_index) {}
