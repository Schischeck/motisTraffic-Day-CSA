...
TEST_F(loader_graph_builder_multiple_ice, eva_num) {
  ...
  EXPECT_STREQ("8000013", stations["8000013"]->eva_nr_.c_str());
  EXPECT_STREQ("8000025", stations["8000025"]->eva_nr_.c_str());
  EXPECT_STREQ("8000078", stations["8000078"]->eva_nr_.c_str());
  EXPECT_STREQ("8000122", stations["8000122"]->eva_nr_.c_str());
  EXPECT_STREQ("8000228", stations["8000228"]->eva_nr_.c_str());
  EXPECT_STREQ("8000260", stations["8000260"]->eva_nr_.c_str());
  EXPECT_STREQ("8000261", stations["8000261"]->eva_nr_.c_str());
  EXPECT_STREQ("8000284", stations["8000284"]->eva_nr_.c_str());
  EXPECT_STREQ("8001844", stations["8001844"]->eva_nr_.c_str());
  EXPECT_STREQ("8004158", stations["8004158"]->eva_nr_.c_str());
  EXPECT_STREQ("8010101", stations["8010101"]->eva_nr_.c_str());
  EXPECT_STREQ("8010205", stations["8010205"]->eva_nr_.c_str());
  EXPECT_STREQ("8010222", stations["8010222"]->eva_nr_.c_str());
  EXPECT_STREQ("8010240", stations["8010240"]->eva_nr_.c_str());
  EXPECT_STREQ("8010309", stations["8010309"]->eva_nr_.c_str());
  EXPECT_STREQ("8011102", stations["8011102"]->eva_nr_.c_str());
  EXPECT_STREQ("8011113", stations["8011113"]->eva_nr_.c_str());
  EXPECT_STREQ("8011956", stations["8011956"]->eva_nr_.c_str());
  EXPECT_STREQ("8098160", stations["8098160"]->eva_nr_.c_str());
}

TEST_F(loader_graph_builder_multiple_ice, simple_test) {
  auto& stations = sched_->eva_to_station_;
  ASSERT_STREQ("Augsburg Hbf", stations["8000013"]->name_.c_str());
  ASSERT_STREQ("Bamberg", stations["8000025"]->name_.c_str());
  ASSERT_STREQ("Donauwörth", stations["8000078"]->name_.c_str());
  ASSERT_STREQ("Treuchtlingen", stations["8000122"]->name_.c_str());
  ASSERT_STREQ("Lichtenfels", stations["8000228"]->name_.c_str());
  ASSERT_STREQ("Würzburg Hbf", stations["8000260"]->name_.c_str());
  ASSERT_STREQ("München Hbf", stations["8000261"]->name_.c_str());
  ASSERT_STREQ("Nürnberg Hbf", stations["8000284"]->name_.c_str());
  ASSERT_STREQ("Erlangen", stations["8001844"]->name_.c_str());
  ASSERT_STREQ("München-Pasing", stations["8004158"]->name_.c_str());
  ASSERT_STREQ("Erfurt Hbf", stations["8010101"]->name_.c_str());
  ASSERT_STREQ("Leipzig Hbf", stations["8010205"]->name_.c_str());
  ASSERT_STREQ("Lutherstadt Wittenberg", stations["8010222"]->name_.c_str());
  ASSERT_STREQ("Naumburg(Saale)Hbf", stations["8010240"]->name_.c_str());
  ASSERT_STREQ("Saalfeld(Saale)", stations["8010309"]->name_.c_str());
  ASSERT_STREQ("Berlin Gesundbrunnen", stations["8011102"]->name_.c_str());
  ASSERT_STREQ("Berlin Südkreuz", stations["8011113"]->name_.c_str());
  ASSERT_STREQ("Jena Paradies", stations["8011956"]->name_.c_str());
  ASSERT_STREQ("Berlin Hbf (tief)", stations["8098160"]->name_.c_str());
}
...
TEST_F(loader_graph_builder_multiple_ice, route_nodes) {
  ...
  for (auto const& first_route_node :
    ...
    if (station_eva == "8000284") {
      ...
      ASSERT_EQ(edge::FOOT_EDGE, first_route_node->incoming_edges_[0]->type());
      ASSERT_EQ(edge::ROUTE_EDGE, first_route_node->edges_[1].type());
      ASSERT_EQ(edge::FOOT_EDGE, first_route_node->edges_[0].type());

      auto next_route_node = first_route_node->edges_[1].to_;
      ASSERT_TRUE(next_route_node->is_route_node());

      ASSERT_STREQ("8000260",
                   sched_->stations_[next_route_node->get_station()->id_]
                       ->eva_nr_.c_str());
      ...
      for (auto const& c : connections) {
        ...
        ASSERT_STREQ("---", fc->con_info_->provider_->short_name_.c_str());
        ASSERT_STREQ("DB AG", fc->con_info_->provider_->long_name_.c_str());
        ASSERT_STREQ("Deutsche Bahn AG",
                     fc->con_info_->provider_->full_name_.c_str());
      }
      ...
      EXPECT_STREQ("BT", bt->code_.c_str());
      EXPECT_STREQ("SN", sn->code_.c_str());
      EXPECT_STREQ("Bordbistro", bt->str_.c_str());
      EXPECT_STREQ("SnackPoint/Imbiss im Zug", sn->str_.c_str());
