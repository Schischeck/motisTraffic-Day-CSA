...
TEST(loader_hrd_stations_parser, parse_stations) {
  try {
    ...
    for(auto const& meta_station : meta_stations) {
      if(meta_station.eva == 8089221) {
        ASSERT_EQ(1, meta_station.equivalent.size());
        EXPECT_EQ(0122662, meta_station.equivalent[0]);
      }

      if(meta_station.eva == 8006552) {
        ASSERT_EQ(2, meta_station.equivalent.size());
        EXPECT_EQ(8006552, meta_station.equivalent[0]);
        EXPECT_EQ(8089222, meta_station.equivalent[1]);
      }
    }
  } catch (parser_error const& pe) {
    pe.print_what();
