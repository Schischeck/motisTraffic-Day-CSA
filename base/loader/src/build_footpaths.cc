...
struct footpath_builder {
  ...
  footpath_builder(unsigned& next_node_id, schedule& sched,
                   bool const& adjust_footpaths,
                   hash_map<Station const*, station_node*> const& station_nodes)
      : next_node_id_(next_node_id),
        sched_{sched},
        adjust_footpaths_{adjust_footpaths},
        station_nodes_{station_nodes} {}
  ...
  void add_footpaths(f::Vector<f::Offset<Footpath>> const* footpaths) {
    ...
    for (auto const& footpath : *footpaths) {
      auto const get_station = [&](char const* tag, Station const* s) {
        ...
        utl::new_verify(it != end(station_nodes_),
                        "footpath " + std::string(tag) + " node not found " +
                            footpath->from()->name()->str() + " [" +
                            footpath->from()->id()->str() + "] ");
        return it->second;
      };
  ...
  node* get_or_create_foot_node(node* n) {
    ...
    if (sn->foot_node_ != nullptr) {
      return sn->foot_node_;
    }

    // Create the foot node.
    auto foot_node = new node(sn, next_node_id_++);

    // STATION_NODE -(FWD_EDGE)-> FOOT_NODE
    sn->edges_.emplace_back(make_fwd_edge(sn, foot_node));

    // FOOT_NODE -(BWD_EDGE)-> STATION_NODE
    foot_node->edges_.emplace_back(make_bwd_edge(foot_node, sn));
      ...
      for (auto const& e : route_node->edges_) {
        if (e.to_ == sn && e.type() != edge::INVALID_EDGE) {
          ...
          route_node->edges_.push_back(
              make_after_train_fwd_edge(route_node, foot_node, 0, true));
          break;
        }
      }
    });
    ...
    for (auto const& e : sn->edges_) {
      if (e.to_->is_route_node() && e.type() != edge::INVALID_EDGE) {
        foot_node->edges_.emplace_back(
            make_after_train_bwd_edge(foot_node, e.to_, 0, true));
      }
    }

    sn->foot_node_ = foot_node;
    return sn->foot_node_;
  }
    ...
    return utl::to_vec(sched_.station_nodes_, [](auto const& station_node) {
      ...
      if (station_node->foot_node_ != nullptr) {
        for (auto const& foot_edge : station_node->foot_node_->edges_) {
          ...
          utl::new_verify(from_station == station_node->id_,
  ...
  void process_component(component_it const lb, component_it const ub,
    ...
    if (size == 2) {
      ...
      if (!fgraph[idx_a].empty()) {
        utl::verify_silent(
            fgraph[idx_a].size() == 1,
            "invalid size (a): idx_a=" + std::to_string(idx_a) +
                ", size=" + std::to_string(fgraph[idx_a].size()) + ", data=[" +
                to_str(fgraph[idx_a]) +
                "], "
                "idx_b=" +
                std::to_string(idx_b) +
                ", size = " + std::to_string(fgraph[idx_b].size()) + " " +
                to_str(fgraph[idx_b]));
      ...
      if (!fgraph[idx_b].empty()) {
        utl::verify_silent(
            fgraph[idx_b].size() == 1,
            "invalid size (a): idx_a=" + std::to_string(idx_a) +
                ", size=" + std::to_string(fgraph[idx_a].size()) +
                ", idx_b=" + std::to_string(idx_b) +
                ", size=" + std::to_string(fgraph[idx_b].size()) + "");
    ...
    utl::new_verify(size > 2, "invalid size");
  ...
  schedule& sched_;
  bool const& adjust_footpaths_;
  hash_map<Station const*, station_node*> const& station_nodes_;
};

void build_footpaths(
    unsigned& next_node_id, schedule& sched, bool const& adjust_footpaths,
    bool const& expand_footpaths,
    hash_map<Station const*, station_node*> const& station_nodes,
    Schedule const* const& fbs_schedule) {
  footpath_builder b{next_node_id, sched, adjust_footpaths, station_nodes};

  b.add_footpaths(fbs_schedule->footpaths());

  // b.equivalences_to_footpaths();  // entfernen (sollte nur gebraucht
  // werden wenn mehrere Fahrpläne genutzt
  // werden)
