...
void railviz::station_info(msg_ptr msg, webclient&, callback cb) {
  ...
  timetable timetable_ = timetable_retriever_.ordered_timetable_for_station( *station_nodes[index].get() );
  ...
  for( const timetable_entry& entry : timetable_ )
  {
    ...
    std::string event_name = lock.sched().category_names[(int)lc->_full_con->clasz] + " ";
    std::string direction = "from: ";
    if( !std::get<3>(entry) )
      direction = "to: ";
    event_name.append(lc->_full_con->con_info->line_identifier.to_string()).append(" ");
    event_name.append(direction).append(lock.sched().stations[end_start_station->_id].get()->name);
    ...
    timetable_fb.push_back(CreateTimetableEntry(b,b.CreateString(event_name),&t,end_start_station->_id,true));
  }
