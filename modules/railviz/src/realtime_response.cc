...
std::pair<unsigned int, unsigned int> realtime_response::delay( const light_connection& lc, const edge& e ) const {
...
std::pair<unsigned int, unsigned int> realtime_response::delay( const route_entry& route_entry_ ) const {
  ...
  const motis::station_node* next_station = next_station_node( *std::get<1>(route_entry_), std::get<1>(route_entry_)->_route );
  delays.first = delay( *std::get<2>(route_entry_), *std::get<0>(route_entry_), true, std::get<1>(route_entry_)->_route );
  delays.second = delay( *std::get<2>(route_entry_), *next_station, false, std::get<1>(route_entry_)->_route );
...
unsigned int realtime_response::delay_batch(const light_connection& lc, const station_node& station_node_, bool departure, unsigned int route_id) const {
  if( batch_rsp == nullptr )
    return 0;

  int num_responses = batch_rsp->trains()->Length();
  for( int i = 0; i < num_responses; i++ ) {
    realtime::RealtimeTrainInfoResponse const* response = batch_rsp->trains()->Get(i);
...
unsigned int realtime_response::delay_single( const motis::realtime::RealtimeTrainInfoResponse& response, const light_connection& lc, const station_node& station_node_, bool departure) const {
...
const motis::station_node* realtime_response::next_station_node( const motis::node& node_, unsigned int route_id ) const {
