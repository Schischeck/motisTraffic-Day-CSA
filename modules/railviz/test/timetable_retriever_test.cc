...
TEST_CASE("14:test case with a loop",
          "[railviz::timetable_retriever::stations_on_route]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
...
TEST_CASE("15:test case with a loop",
          "[railviz::timetable_retriever::get_routes_on_time]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
...
TEST_CASE("13:test case with a loop",
          "[railviz::timetable_retriever::ordered_timetable_for_station]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
...
TEST_CASE("11:test case with a loop",
          "[railviz::timetable_retriever::timetable_for_station_outgoing]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
  ...
  SECTION("Time table for station node 02 should be correct") {
    ...
    REQUIRE(std::get<1>(res_tt_02[28]) ==
            st_02_stnode->get_route_nodes().at(1)->get_station());
    REQUIRE(std::get<2>(res_tt_02[28]) ==
            ttr.child_node(*(st_02_stnode->get_route_nodes().at(1)))
                ->get_station());
    REQUIRE(std::get<3>(res_tt_02[28]) == st_05_routenode->get_station());
    REQUIRE(std::get<4>(res_tt_02[28]) == true);
    REQUIRE(std::get<5>(res_tt_02[28]) ==
...
TEST_CASE("12:test case with a loop",
          "[railviz::timetable_retriever::timetable_for_station_incoming]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
  ...
  SECTION("Time table for station node 02 should be correct") {
    ...
    REQUIRE(std::get<1>(res_tt_02[0]) ==
            st_02_stnode->get_route_nodes().at(0)->get_station());
    REQUIRE(std::get<2>(res_tt_02[0]) ==
            ttr.parent_node(*(st_02_stnode->get_route_nodes().at(0)))
                ->get_station());
    ...
    REQUIRE(std::get<1>(res_tt_02[28]) ==
            st_02_stnode->get_route_nodes().at(1)->get_station());
    REQUIRE(std::get<2>(res_tt_02[28]) ==
            ttr.parent_node(*(st_02_stnode->get_route_nodes().at(1)))
                ->get_station());
...
TEST_CASE("01:test case without loop",
          "[railviz::timetable_retriever::parent_node]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/01_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
...
TEST_CASE("02:test case with a loop",
          "[railviz::timetable_retriever::parent_node]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  ...
  SECTION(
      "02 (loop path) route node should have a parent equals to 04 route "
      "node") {
    REQUIRE(ttr.parent_node(*st_02_loop_routenode) == st_04_routenode);
  }

  SECTION(
      "05 route node should have a parent equals to 02 (loop path) route "
      "node") {
...
TEST_CASE("03:test case without loop",
          "[railviz::timetable_retriever::child_node]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/01_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
...
TEST_CASE("04:test case with a loop",
          "[railviz::timetable_retriever::child_node]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  ...
  SECTION(
      "04 route node should have a child equals to 02 (loop path) route node") {
    REQUIRE(ttr.child_node(*st_04_routenode) == st_02_loop_routenode);
  }

  SECTION(
      "02 (loop path) route node should have a child equals to 05 route node") {
...
TEST_CASE("05:test case without loop",
          "[railviz::timetable_retriever::start_node_for_route]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/01_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
...
TEST_CASE("06:test case with a loop",
          "[railviz::timetable_retriever::start_node_for_route]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  ...
  SECTION(
      "02 (loop path) route node should belong to a route with the start node "
      "01") {
...
TEST_CASE("07:test case without loop",
          "[railviz::timetable_retriever::end_node_for_route]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/01_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
...
TEST_CASE("08:test case with a loop",
          "[railviz::timetable_retriever::end_node_for_route]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  ...
  SECTION(
      "02 (loop path) route node should belong to a route with the end node "
      "05") {
...
TEST_CASE("09:test case without loop",
          "[railviz::timetable_retriever::get_route_depature_times]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/01_test_set/motis");
  timetable_retriever ttr;
  std::map<int, motis::station*>::iterator it;
  ...
  SECTION(
      "01 route node should belong to a route with the ref_vector of "
      "d_time's") {
    REQUIRE(ttr.get_route_departure_times(*st_01_routenode) == ref_vector);
  }

  SECTION(
      "02 route node should belong to a route with the ref_vector of "
      "d_time's") {
    REQUIRE(ttr.get_route_departure_times(*st_02_routenode) == ref_vector);
  }

  SECTION(
      "03 route node should belong to a route with the ref_vector of "
      "d_time's") {
    REQUIRE(ttr.get_route_departure_times(*st_03_routenode) == ref_vector);
  }
}

TEST_CASE("10:test case with a loop",
          "[railviz::timetable_retriever::get_route_depature_times]") {
  auto schedule = motis::load_schedule(
      "../test_timetables/timetable_retriever_test/02_test_set/motis");
  timetable_retriever ttr;
  ...
  SECTION(
      "04 route node should belong to a route with the ref_vector of "
      "d_time's") {
    REQUIRE(ttr.get_route_departure_times(*st_04_routenode) == ref_vector);
  }

  SECTION(
      "02 (loop path) route node should belong to a route with the ref_vector "
      "of d_time's") {
    REQUIRE(ttr.get_route_departure_times(*st_02_loop_routenode) == ref_vector);
  }

  SECTION(
      "05 route node should belong to a route with the ref_vector of "
      "d_time's") {
