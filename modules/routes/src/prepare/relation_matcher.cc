...
void relation_matcher::load_osm() {
  ...
  for (auto &rel : relations_) {
    ...
    for (auto const &w : rel.second.ways_) {
      auto const way = ways_.at(w);
      ways.emplace_back(way.id_, way.nodes_.front(), way.nodes_.back());
    }
    auto const &sorted_ways = sort_ways(ways);
    rel.second.ways_.clear();
    for (auto const &w : sorted_ways) {
      auto way = ways_.at(w);
      rel.second.ways_.push_back(w);
      rel.second.nodes_.insert(rel.second.nodes_.end(), way.nodes_.begin(),
                               way.nodes_.end());
    }
  }
