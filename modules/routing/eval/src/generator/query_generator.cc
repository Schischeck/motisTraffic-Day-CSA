...
std::string query(int id, std::time_t interval_start, std::time_t interval_end,
  ...
  auto const path = std::vector<Offset<String>>({fbb.CreateString(to_eva)});
  auto const additional_edges = std::vector<Offset<AdditionalEdgeWrapper>>();
  fbb.create_and_finish(
      MsgContent_RoutingRequest,
      CreateRoutingRequest(
          fbb, Start_PretripStart,
          CreatePretripStart(fbb, fbb.CreateString(from_eva), &interval)
              .Union(),
          fbb.CreateVector(path), fbb.CreateVector(additional_edges))
          .Union(),
      "/routing");
