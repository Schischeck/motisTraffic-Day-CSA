...
void handle_addition(context& ctx, AdditionMessage const*) {
  ctx.stats.additional.inc();

  // TODO

  // auto eva_nr = id->base()->station_id()->str();
  // auto station_id = get_station(ctx.sched, eva_nr)->index;
  // auto train_nr = id->base()->service_num();
  // auto motis_time = unix_to_motistime(ctx.sched, id->base()->schedule_time());
