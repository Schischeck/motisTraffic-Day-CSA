...
struct rail_node {
  rail_node(size_t idx, int64_t id) : idx_(idx), id_(id) {}
...
struct rail_edge {
  rail_edge(uint64_t info_idx, bool forward, size_t dist, rail_node const* from,
            rail_node const* to)
      : info_idx_(info_idx),
        forward_(forward),
        dist_(dist),
...
struct rail_edge_info {
  ...
  size_t from_idx(bool forward = true) const {
    if (forward) {
      return nodes_.first->idx_;
    } else {
      return nodes_.second->idx_;
    }
  }

  size_t to_idx(bool forward = true) const {
    if (forward) {
      return nodes_.second->idx_;
    } else {
      return nodes_.first->idx_;
