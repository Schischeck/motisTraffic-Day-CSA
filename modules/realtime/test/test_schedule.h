...
public:
  ...
  std::vector<motis::journey> find_connections(const motis::station* from,
                                               ...
                                               int interval = 60) {
    int day = departure_begin / MINUTES_A_DAY;
    int min_begin = departure_begin % MINUTES_A_DAY;
    int min_end = min_begin + interval;
    ...
    std::vector<motis::journey> journeys = _search.get_connections(
        {start}, {target}, min_begin, min_end, day, &stats);
  ...
  void check_stops(const motis::journey& journey,
    ...
    for (std::size_t i = 0; i < expected_stops.size(); i++) {
      ...
      CHECK(jstop.eva_no == estop.station->eva_nr.to_string());
