...
void motis::realtime::handler::handle_reroute(
  ...
  rt_msg.canceled_events_.reserve(ris_msg->cancelledEvents()->size());
  rt_msg.new_events_.reserve(ris_msg->newEvents()->size());

  for (auto canceled_event : *ris_msg->cancelledEvents()) {
    rt_msg.canceled_events_.emplace_back(
        ris_event_to_schedule_event(*canceled_event, ctx._schedule));
  }
  for (auto new_event : *ris_msg->newEvents()) {
    rt_msg.new_events_.emplace_back(
        ris_event_to_schedule_event(*new_event->base()->base(), ctx._schedule));
  }

  if (ris_msg->newEvents()->size() > 0) {
    rt_msg.category_ =
        ris_msg->newEvents()->Get(0)->base()->trainCategory()->str();
