...
const station* message_handler::get_station_by_eva(std::string eva_nr) const {
  try {
    ...
    auto it = _eva_station_map.find(eva);
    return it == std::end(_eva_station_map) ? nullptr : it->second;
  } catch (boost::bad_lexical_cast&) {
    return nullptr;
  }
}

void message_handler::init_station_map() {
  for (const auto& s : _rts._schedule.stations) {
    try {
      int eva_nr = boost::lexical_cast<int>(s->eva_nr.to_string());
      _eva_station_map[eva_nr] = s.get();
    } catch (boost::bad_lexical_cast&) {
      std::cerr << "invalid eva nr " << s->eva_nr << " for station" << s->name
                << "\n";
    }
  }
}
