...
struct train_station_info {
  ...
  void write(ostream& out, bool first, bool last) const {
    if (!first) out << "a" << a_time << " " << a_platform;
    if (!first && !last) out << " ";
    if (!last) {
      out << "d" << d_time << " " << d_platform << " " << traffic_day_index
...
struct route_station_info {
  ...
  bool operator<(const route_station_info& o) const {
    ...
    if (skip_arrival != o.skip_arrival) return skip_arrival < o.skip_arrival;
  ...
  void write(ostream& out, bool first, bool last) const {
    out << "l" << location;
    if (!first) out << " a" << skip_arrival;
    if (!last) out << " d" << skip_departure << " " << family;
  }
};
...
struct route_info {
  ...
  bool operator==(const route_info& o) const { return stations == o.stations; }

  bool operator<(const route_info& o) const {
    unsigned int n = stations.size(), n2 = o.stations.size();
    if (n != n2) return n < n2;
    for (unsigned int i = 0; i < n; ++i)
      if (!(stations[i] == o.stations[i])) return stations[i] < o.stations[i];
    return false;
  }
  ...
  */
  vector<int> get_traffic_day_shifts() {
    ...
    for (unsigned int i = 1; i < stations.size() - 1; ++i) {
      if (stations[i].d_time < stations[i - 1].d_time) ++shifts;
      ret[i] = shifts;
    }
  ...
  vector<service_info> split(bitset_manager& bm) {
    ...
    // check for each pair of adjacent stations whether their traffic days
    // differ
    for (unsigned int i = 0; i < stations.size() - 2; ++i) {
      ...
      if (!bm.equals(
              bm.shift_l(stations[i].traffic_day_index, shifts[i]),
              bm.shift_l(stations[i + 1].traffic_day_index, shifts[i + 1]),
              shifts[i + 1] - shifts[i])) {
        int common_traffic_days = bm.common_bitset(
            bm.shift_l(stations[i].traffic_day_index, shifts[i]),
            bm.shift_l(stations[i + 1].traffic_day_index, shifts[i + 1]));
        ...
        if (traffic_day_index != EMPTY_BITSET) vsi.push_back(si2);
    ...
    if (stations[0].traffic_day_index != EMPTY_BITSET) vsi.push_back(*this);
  ...
  service_info remove_traffic_days(int index, bitset_manager& bm) {
    ...
    for (unsigned int i = 0; i < stations.size() - 1; ++i)
      ret.stations[i].traffic_day_index = bm.without(
          ret.stations[i].traffic_day_index, bm.shift_r(index, shifts[i]));
    return ret;
  }

  /**
   * merges the two service_infos and returns the number of resulting
   *service_info
   * objects.
   ...
   **/
  int merge(/*in/out*/ service_info& o, /*out*/ service_info& o2,
            bitset_manager& bm) {
    int size = stations.size();
    assert(size > 1);
    // the two services have to be connectible
    if (stations[size - 1].location != o.stations[0].location) return 2;

    int shifts = 0;
    // day/night shift?
    if (stations[size - 2].d_time > o.stations[0].d_time) shifts = 1;
    ...
    if (bm.equals(index1, index2, shifts)) {
      ...
      if (bm.equals(common, EMPTY_BITSET, shifts)) return 2;
...
void read_station(istream& in, service_station_info& ssi, bool first_or_last) {
  ...
  if (c == 'd') {
    ...
    for (unsigned int i = 0; i < n; ++i) in >> ssi.attributes[i];
    in >> ssi.train_nr;
    in >> c;
    assert(c == '%');
...
int read_service(istream& in, vector<vector<service_info>>& services,
  ...
  if (c == 't') {
    ...
    for (int i = 0; i < ntimes; ++i) in >> csi.times[i];
    ...
    for (int i = 0; i < n_bitfields; ++i) in >> csi.bitfields[i];
...
void extract_route_info(const service_info& si, route_info& ri,
                        train_info& ti) {
...
void services2routes(std::string const& prefix) {
  ...
  while (!in.eof() && in.peek() != EOF) {
    ...
    if (index == -1)
      ...
      if (last_i % 5000 == 0) LOG(debug) << last_i << " services read";
    }
  }
  in.close();
  ...
  int count = 0;
  for (unsigned int i = 0; i <= last_i; ++i) count += services[i].size();
  ...
  for (unsigned int i = 0; i <= last_i; ++i) {
    ...
    if (i % 5000 == 0 && i != 0) LOG(debug) << i << " services splitted";
  }
  ...
  for (unsigned int i = 0; i <= last_i; ++i) {
    ...
    count += services[i].size();
    if (i % 5000 == 0 && i != 0) LOG(info) << i << " services merged";
  }
  ...
  if (!rout) throw std::runtime_error("routes file for writing");
  int curr_index = 0;
