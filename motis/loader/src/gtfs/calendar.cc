...
std::bitset<7> traffic_week_days(gtfs_calendar const& c) {
  std::bitset<7> days;
  days.set(0, get<monday>(c) == 1 ? true : false);
  days.set(1, get<tuesday>(c) == 1 ? true : false);
  days.set(2, get<wednesday>(c) == 1 ? true : false);
  days.set(3, get<thursday>(c) == 1 ? true : false);
  days.set(4, get<friday>(c) == 1 ? true : false);
  days.set(5, get<saturday>(c) == 1 ? true : false);
  days.set(6, get<sunday>(c) == 1 ? true : false);
  return days;
}
...
std::map<std::string, calendar> read_calendar(loaded_file file) {
  ...
  for (auto const& c : read<gtfs_calendar>(file.content, calendar_columns)) {
    services.insert(std::make_pair(
        ...
        calendar{traffic_week_days(c), get<start_date>(c), get<end_date>(c)}));
  }
  return services;
}
