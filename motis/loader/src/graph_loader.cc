...
void graph_loader::load_waiting_time_rules(
  ...
  in >> number_of_groups >> rules.default_group;
  ...
  for (int i = 0; i < number_of_groups; i++) {
    ...
    in >> group_id >> sep >> categories;

    rules.add_category(group_id, categories);
  }
  ...
  for (int i = 0; i < number_of_groups * number_of_groups; i++) {
    ...
    if (waiting_time > 0) {
      ...
      rules._other_trains_wait_for[feeder_category] = true;
    }
  }
