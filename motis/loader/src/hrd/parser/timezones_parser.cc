...
int bitfield_idx(cstr ddmmyyyy, date const& first_schedule_date) {
  date season_begin_date(parse<int>(ddmmyyyy.substr(0, size(4))),
                         parse<int>(ddmmyyyy.substr(4, size(2))),
                         parse<int>(ddmmyyyy.substr(6, size(2))));
...
timezones parse_timezones(loaded_file const& timezones_file,
  ...
  for_each_line(timezones_file.content(), [&](cstr line) {
    if (line.length() == 15) {
      ...
      auto it = tz.eva_num_to_tz_entry.lower_bound(first_valid_eva_number);
      verify(it != end(tz.eva_num_to_tz_entry),
             "missing timezone information for eva number: %d",
             first_valid_eva_number);

      tz.eva_num_to_tz_entry[eva_number(line.substr(0, size(7)))] = it->second;
      return;
    }
    if (isdigit(line[0]) && line.length() >= 47) {
      tz.timezone_entries_.push_back(make_unique<timezone_entry>(
          distance_to_midnight(line.substr(8, size(5))),
      ...
      tz.eva_num_to_tz_entry[eva_number(line.substr(0, size(7)))] =
