...
bitset_index bitset_manager::common_bitset(bitset_index first,
                                           bitset_index second) {
  schedule_bitset bs;
  bs |= _bitsets[first];
  bs &= _bitsets[second];
...
bitset_index bitset_manager::difference_bitset(bitset_index larger,
                                               bitset_index smaller) {
  schedule_bitset bs;
  bs |= _bitsets[larger];
  bs &= _bitsets[smaller];
  schedule_bitset bs2;
...
bitset_index bitset_manager::shift_l(bitset_index i, int amount) {
  ...
  bs |= _bitsets[i];
  bs <<= amount;
  for (int j = 0; j < amount; ++j) bs[j] = bs[_real_size + j] = 0;
  return add(bs);
}
...
int bitset_manager::get_next_possible_day(bitset_index bi, int day) const {
  while (day <= _real_size && _bitsets[bi][_real_size - day] == 0) ++day;
  if (day > _real_size)
    return -1;
  else
    return day;
...
std::vector<int> bitset_manager::get_active_day_indices(bitset_index bi) const {
  ...
  for (int day_index = 0; day_index < _real_size; ++day_index)
    if (has_day_i(bi, day_index)) ret.emplace_back(day_index);
  return ret;
}
