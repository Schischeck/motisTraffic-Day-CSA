...
schedule_ptr load_text_schedule(std::string const& prefix)
{
  ...
  node_id = loader.load_routes(node_id, s->stations, s->station_nodes,
                             build_category_class_map(s->category_names, classes),
                             s->full_connections, s->connection_infos,
                             s->route_index_to_first_route_node);
