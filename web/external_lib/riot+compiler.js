...
var mkdom = (function (checkIE) {
  ...
  function _rootel(tagName, ns) {
    if (ns.indexOf('svg') !== -1)
      return 'g'
    else
      return rootEls[tagName] || GENERIC
  }

  // creates any dom element in a div, table, or colgroup container
  function _mkdom(html, ns) {

    var match = html && html.match(/^\s*<([-\w]+)/),
      tagName = match && match[1].toLowerCase(),
      rootTag = _rootel(tagName, ns),
      el = mkEl(rootTag, ns)
    ...
    if (checkIE && tagName && (match = tagName.match(SPECIAL_TAGS_REGEX)))
      ie9elem(el, html, tagName, !!match[1], ns)
    else
      el.innerHTML = html

    if (el.firstChild.hasAttribute('replace-root'))
      el = el.firstChild
...
function Tag(impl, conf, innerHTML) {
  ...
  var self = riot.observable(this),
    ...
    parent = conf.parent,
    parentElement = conf.root.parentElement,
    isLoop = conf.isLoop,
  ...
  if (dom.hasAttribute('replace-root')) {
    root = dom
    if (!isLoop) {
      conf.root.remove()
  ...
  defineProperty(this, 'mount', function() {
    ...
    } else if (dom.hasAttribute('replace-root')) {
      self.dom = dom;
    } else {
...
function setAttr(dom, name, val) {
  name = name.startsWith('late-') ? name.substr(5) : name
...
function mkEl(name, ns) {
  if (typeof document.createElementNS === T_FUNCTION)
    return document.createElementNS(ns || 'http://www.w3.org/1999/xhtml', name)
  else
