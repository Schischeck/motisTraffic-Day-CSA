#ifndef TD_EXECUTION_CHANNEL_H_
#define TD_EXECUTION_CHANNEL_H_
...
public:
  ...
  public:
    ...
    bool operator >> (queue_element& el) {
      std::unique_lock<std::mutex> synchronization(ch_.mutex_);
    ...
    bool operator << (Args... arg) {
      ...
      if (ch_.queue_.size() >= ch_.size_) {
        ch_.cv_.wait(synchronization, [this]() {
          return ch_.queue_.size() < ch_.size_;
        });
      }
    ...
    bool is_data_available() {
      return next_data() != end(ch_.queue_);
    }

    void listen(callback_function cb) {
      std::lock_guard<std::mutex> synchronization(ch_.mutex_);
  ...
  topic operator[](TopicId id) {
    return topic(*this, id);
  }

  void stop() {
    std::lock_guard<std::mutex> synchronization(mutex_);
    stopped_ = true;
    cv_.notify_all();
  }
