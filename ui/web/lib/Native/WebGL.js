...
var _elm_community$webgl$Native_WebGL = function() {
  ...
  function guid() { return _elm_lang$core$Native_Utils.guid(); }
  function listLength(list) { return _elm_lang$core$List$length(list); }
  function listMap(fn, list) { return A2(_elm_lang$core$List$map, fn, list); }
  ...
  function unsafeCoerceGLSL(src) { return {src: src}; }
  ...
  function loadTextureWithFilter(filter, source) {
    ...
    return Scheduler.nativeBinding(function(callback) {
      ...
      img.onerror = function() { callback(Scheduler.fail({ctor: 'Error'})); };
  ...
  function doTexture(gl, texture) {
    ...
    gl.texImage2D(
        gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, texture.img());
  ...
  function doBindAttribute(gl, attribute, bufferElems, elemSize) {
    ...
    var dataIdx = 0;
    var array = new attributeInfo.type(
        listLength(bufferElems) * attributeInfo.size * elemSize);
    ...
    function drawEntity(render) {
      ...
      for (var i = 0; i < numAttributes; i += 1) {
        ...
        if (buffer.buffers[attribute.name] === undefined) {
          buffer.buffers[attribute.name] = doBindAttribute(
              gl, attribute, render.buffer._0, renderType.elemSize);
        }
        ...
        gl.vertexAttribPointer(
            attribLocation, attributeInfo.size, attributeInfo.baseType, false,
            0, 0);
      }
  ...
  function createUniformSetters(gl, model, program) {
    ...
    function createUniformSetter(program, uniform) {
      ...
      switch (uniform.type) {
        case gl.INT:
          return function(value) { gl.uniform1i(uniformLocation, value); };
        case gl.FLOAT:
          return function(value) { gl.uniform1f(uniformLocation, value); };
        case gl.FLOAT_VEC2:
          return function(value) { gl.uniform2fv(uniformLocation, value); };
        case gl.FLOAT_VEC3:
          return function(value) { gl.uniform3fv(uniformLocation, value); };
        case gl.FLOAT_VEC4:
          return function(value) { gl.uniform4fv(uniformLocation, value); };
        case gl.FLOAT_MAT4:
  ...
  function sampleCoverage(value, invert) {
    return function(gl) { gl.sampleCoverage(value, invert); };
  }

  function stencilFunc(func, ref, mask) {
    return function(gl) { gl.stencilFunc(gl[func], ref, mask); };
  }
  ...
  function stencilOperation(fail, zfail, zpass) {
    return function(gl) { gl.stencilOp(gl[fail], gl[zfail], gl[zpass]); };
  }
  ...
  function toHtml(functionCalls, factList, renderables, offscreenRenderables) {
    ...
    return _elm_lang$virtual_dom$Native_VirtualDom.custom(
        factList, model, implementation);
  }

  var implementation = {render: renderCanvas, diff: diff};
  ...
  function renderCanvas(model) {
    ...
    if (gl) {
      ...
      canvas.innerHTML =
          '<a href="http://get.webgl.org/">Enable WebGL</a> to see this content!';
    }
  ...
  function diff(oldModel, newModel) {
    ...
    return {applyPatch: drawGL, data: newModel};
  }
...
var _user$project$Native_WebGL = _elm_community$webgl$Native_WebGL;
